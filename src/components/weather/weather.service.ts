...
class WeatherService {
    static getZoneAirQualityParmsChecker(req:Request,res:Response,next:NextFunction){
    ...
    static async getAndSaveParisAirQuality(req:Request,res:Response){
        ...
        try{
            ...
            if(error.code==='ENOTFOUND'){
                res.status(599).send({status:'failed',message:'Network Connection Timeout Error'})
            }else{
